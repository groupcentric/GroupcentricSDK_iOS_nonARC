...
@protocol gc_SDWebImageDecoderDelegate;

/**
 * Decoding image data is the most expensive step, and it is performed on the main thread. gc_SDWebImageDecoder force the
 * image decoding in a separate thread so UIImage will have high chance to reuse the cached result when used by UI in
 * the main thread.
 *
 * @see https://github.com/rs/gc_SDWebImage/pull/18
 */

@interface gc_SDWebImageDecoder : NSObject
{
 ...
 *
 * @return An gc_SDWebImageDecoder shared instance
 */
...
/**
 * Delegate protocol for gc_SDWebImageDecoder
 */
@protocol gc_SDWebImageDecoderDelegate <NSObject>
 ...
 */
- (void)imageDecoder:(gc_SDWebImageDecoder *)decoder didFinishDecodingImage:(UIImage *)image userInfo:(NSDictionary *)userInfo;
