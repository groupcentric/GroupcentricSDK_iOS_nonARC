...
// gc_MWPhotoBrowser
@implementation gc_MWPhotoBrowser
...
- (id)initWithPhotos:(NSArray *)photosArray andStrings:(NSArray *)photoStringsArray andName:(NSString *)name {
	if ((self = [super init])) {
        ...
        photoStrings = [photoStringsArray retain];
		planTitle = [name retain];
...
- (void)viewDidLoad {    
    ...
    //if (photos.count > 1) {
        ...
        // Toolbar Items
        previousButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"gc_UIBarButtonItemArrowLeft.png"] style:UIBarButtonItemStylePlain target:self action:@selector(gotoPreviousPage)];
        nextButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"gc_UIBarButtonItemArrowRight.png"] style:UIBarButtonItemStylePlain target:self action:@selector(gotoNextPage)];
...
- (void)performLayout {
	...
	for (gc_ZoomingScrollView *page in visiblePages) {
...
- (UIImage *)imageAtIndex:(NSUInteger)index {
	if (photos && index < photos.count) {
		...
		gc_MWPhoto *photo = [photos objectAtIndex:index];
...
#pragma mark -
#pragma mark gc_MWPhotoDelegate
...
- (void)photoDidFailToLoad:(gc_MWPhoto *)photo {
	...
	if (index != NSNotFound) {
		if ([self isDisplayingPageForIndex:index]) {
			...
			// Tell page it failed
			gc_ZoomingScrollView *page = [self pageDisplayedAtIndex:index];
...
- (void)tilePages {
	...
	for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
		if (![self isDisplayingPageForIndex:index]) {
			gc_ZoomingScrollView *page = [self dequeueRecycledPage];
			if (!page) {
				page = [[[gc_ZoomingScrollView alloc] init] autorelease];
...
- (gc_ZoomingScrollView *)pageDisplayedAtIndex:(NSUInteger)index {
	gc_ZoomingScrollView *thePage = nil;
	for (gc_ZoomingScrollView *page in visiblePages) {
		if (page.index == index) {
			thePage = page; break;
		}
...
- (gc_ZoomingScrollView *)dequeueRecycledPage {
	gc_ZoomingScrollView *page = [recycledPages anyObject];
	if (page) {
...
- (void)didStartViewingPageAtIndex:(NSUInteger)index {
    ...
    if (index > 0) {
        ...
        for (i = 0; i < index-1; i++) { [(gc_MWPhoto *)[photos objectAtIndex:i] releasePhoto]; /*NSLog(@"Release image at index %i", i);*/ }
        
        // Preload index - 1
        i = index - 1; 
        if (i < photos.count) { [(gc_MWPhoto *)[photos objectAtIndex:i] obtainImageInBackgroundAndNotify:self]; /*NSLog(@"Pre-loading image at index %i", i);*/ }
        
    }
    if (index < photos.count - 1) {
        
        // Release anything > index + 1
        for (i = index + 2; i < photos.count; i++) { [(gc_MWPhoto *)[photos objectAtIndex:i] releasePhoto]; /*NSLog(@"Release image at index %i", i);*/ }
...
- (CGRect)frameForPagingScrollView {
    CGRect frame = self.view.bounds; //[[UIScreen mainScreen] bounds]; // self.view.bounds
...
- (void)setControlsHidden:(BOOL)hidden {
	...
	[self.navigationController.navigationBar setAlpha:hidden ? 0 : 0.8];
...
- (void)share {
    UIActionSheet *ac = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:@"Save Image", nil];
    ac.tag = 1;
...
#pragma mark -
#pragma mark UIActionSheet Delegation
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (actionSheet.tag == 1) {
        if (buttonIndex == 0) {
            ...
            gc_MWPhoto *photo = [photos objectAtIndex:currentPageIndex];
            if ([photo photoImage]) {
