...
static gc_SDImageCache *instance;
...
@implementation gc_SDImageCache
...
- (id)init
{
    if ((self = [super init]))
    {
        ...
        diskCachePath = gc_SDWIReturnRetained([[paths objectAtIndex:0] stringByAppendingPathComponent:@"ImageCache"]);
...
- (void)dealloc
{
    gc_SDWISafeRelease(memCache);
    gc_SDWISafeRelease(diskCachePath);
    gc_SDWISafeRelease(cacheInQueue);
...
#pragma mark gc_SDImageCache (class methods)

+ (gc_SDImageCache *)sharedImageCache
{
    if (instance == nil)
    {
        instance = [[gc_SDImageCache alloc] init];
    }
...
#pragma mark gc_SDImageCache (private)
...
- (void)storeKeyWithDataToDisk:(NSArray *)keyAndData
{
    ...
    else
    {
        ...
        UIImage *image = gc_SDWIReturnRetained([self imageFromKey:key fromDisk:YES]); // be thread safe with no lock
        if (image)
        {
#if TARGET_OS_IPHONE
...
#endif
            gc_SDWIRelease(image);
        }
    }

    gc_SDWIRelease(fileManager);
}

- (void)notifyDelegate:(NSDictionary *)arguments
{
    NSString *key = [arguments objectForKey:@"key"];
    id <gc_SDImageCacheDelegate> delegate = [arguments objectForKey:@"delegate"];
...
{
    NSString *key = [arguments objectForKey:@"key"];
    NSMutableDictionary *mutableArguments = gc_SDWIReturnAutoreleased([arguments mutableCopy]);

    UIImage *image = gc_SDScaledImageForPath(key, [NSData dataWithContentsOfFile:[self cachePathForKey:key]]);
...
- (void)storeImage:(UIImage *)image imageData:(NSData *)data forKey:(NSString *)key toDisk:(BOOL)toDisk
{
    ...
    if (toDisk)
    {
        ...
        NSInvocationOperation *operation = gc_SDWIReturnAutoreleased([[NSInvocationOperation alloc] initWithTarget:self
...
- (UIImage *)imageFromKey:(NSString *)key fromDisk:(BOOL)fromDisk
{
    ...
    if (!image && fromDisk)
    {
        image = gc_SDScaledImageForPath(key, [NSData dataWithContentsOfFile:[self cachePathForKey:key]]);
        if (image)
        {
            if (get_free_memory() < minFreeMemLeft)
            {
...
- (void)queryDiskCacheForKey:(NSString *)key delegate:(id <gc_SDImageCacheDelegate>)delegate userInfo:(NSDictionary *)info
{
    if (!delegate)
    {
        return;
