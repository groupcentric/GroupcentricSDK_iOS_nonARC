...
@class UA_ASIDataDecompressor;

extern NSString *UA_ASIHTTPRequestVersion;
...
// The error domain that all errors generated by UA_ASIHTTPRequest use
extern NSString* const UA_NetworkRequestErrorDomain;
...
extern unsigned long const UA_ASIWWANBandwidthThrottleAmount;

#if NS_BLOCKS_AVAILABLE
typedef void (^UA_ASIBasicBlock)(void);
typedef void (^UA_ASIHeadersBlock)(NSDictionary *responseHeaders);
typedef void (^UA_ASISizeBlock)(long long size);
typedef void (^UA_ASIProgressBlock)(unsigned long long size, unsigned long long total);
typedef void (^UA_ASIDataBlock)(NSData *data);
#endif

@interface UA_ASIHTTPRequest : NSOperation <NSCopying> {
	...
	// The delegate - will be notified of various changes in state via the UA_ASIHTTPRequestDelegate protocol
	...
	// Generally, you won't use this directly, but UA_ASINetworkQueue sets itself as the queue so it can proxy updates to its own delegates
	...
	// Automatically set to true in UA_ASIFormDataRequests when using setFile:forKey:
	...
	// Set to true when UA_ASIHTTPRequest automatically created a temporary file containing the request body (when true, the file at postBodyFilePath will be deleted at the end of the request)
	...
	// If you are using Basic authentication and want to force UA_ASIHTTPRequest to send an authorization header without waiting for a 401, you must set this to (NSString *)kCFHTTPAuthenticationSchemeBasic
	NSString *authenticationScheme;
	
	// Realm for authentication when credentials are required
	NSString *authenticationRealm;
	
	// When YES, UA_ASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a server that requires authentication
	...
	// When YES, UA_ASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a proxy server that requires authentication
	...
	// If you set this and implement the method in your delegate, you must handle the data yourself - UA_ASIHTTPRequest will not populate responseData or write the data to downloadDestinationPath
	...
	// Also see the comments in UA_ASINetworkQueue.h
	...
	// Tells UA_ASIHTTPRequest not to delete partial downloads, and allows it to use an existing file to resume a download. Defaults to NO.
	...
	// Details on the proxy to use - you could set these yourself, but it's probably best to let UA_ASIHTTPRequest detect the system proxy settings
	NSString *proxyHost;
	int proxyPort;
	
	// UA_ASIHTTPRequest will assume kCFProxyTypeHTTP if the proxy type could not be automatically determined
	...
	// See UA_ASIAuthenticationState values above. 0 == default == No authentication needed yet
	UA_ASIAuthenticationState authenticationNeeded;
	
	// When YES, UA_ASIHTTPRequests will present credentials from the session store for requests to the same server before being asked for them
	...
	// Temporarily set to YES when a closed connection forces a retry (internally, this stops UA_ASIHTTPRequest cleaning up a temporary post body)
	...
	// Will be UA_ASIHTTPRequestRunLoopMode for synchronous requests, NSDefaultRunLoopMode for all other requests
	NSString *runLoopMode;
	
	// This timer checks up on the request every 0.25 seconds, and updates progress
	NSTimer *statusTimer;
	
	// The download cache that will be used for this request (use [UA_ASIHTTPRequest setDefaultCache:cache] to configure a default cache
	id <UA_ASICacheDelegate> downloadCache;
	
	// The cache policy that will be used for this request - See UA_ASICacheDelegate.h for possible values
	UA_ASICachePolicy cachePolicy;
	
	// The cache storage policy that will be used for this request - See UA_ASICacheDelegate.h for possible values
	...
	//
	// Setting this to NO may be especially useful for users using UA_ASIHTTPRequest in conjunction with a streaming parser, as it will allow partial gzipped responses to be inflated and passed on to the parser while the request is still running
	...
	UA_ASIHTTPRequest *PACFileRequest;
	...
	UA_ASIBasicBlock startedBlock;

	//block to execute when headers are received
	UA_ASIHeadersBlock headersReceivedBlock;

	//block to execute when request completes successfully
	UA_ASIBasicBlock completionBlock;

	//block to execute when request fails
	UA_ASIBasicBlock failureBlock;

	//block for when bytes are received
	UA_ASIProgressBlock bytesReceivedBlock;

	//block for when bytes are sent
	UA_ASIProgressBlock bytesSentBlock;

	//block for when download size is incremented
	UA_ASISizeBlock downloadSizeIncrementedBlock;

	//block for when upload size is incremented
	UA_ASISizeBlock uploadSizeIncrementedBlock;

	//block for handling raw bytes received
	UA_ASIDataBlock dataReceivedBlock;

	//block for handling authentication
	UA_ASIBasicBlock authenticationNeededBlock;

	//block for handling proxy authentication
	UA_ASIBasicBlock proxyAuthenticationNeededBlock;
	
    //block for handling redirections, if you want to
    UA_ASIBasicBlock requestRedirectedBlock;
	#endif
}
...
+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <UA_ASICacheDelegate>)cache;
+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <UA_ASICacheDelegate>)cache andCachePolicy:(UA_ASICachePolicy)policy;

#if NS_BLOCKS_AVAILABLE
- (void)setStartedBlock:(UA_ASIBasicBlock)aStartedBlock;
- (void)setHeadersReceivedBlock:(UA_ASIHeadersBlock)aReceivedBlock;
- (void)setCompletionBlock:(UA_ASIBasicBlock)aCompletionBlock;
- (void)setFailedBlock:(UA_ASIBasicBlock)aFailedBlock;
- (void)setBytesReceivedBlock:(UA_ASIProgressBlock)aBytesReceivedBlock;
- (void)setBytesSentBlock:(UA_ASIProgressBlock)aBytesSentBlock;
- (void)setDownloadSizeIncrementedBlock:(UA_ASISizeBlock) aDownloadSizeIncrementedBlock;
- (void)setUploadSizeIncrementedBlock:(UA_ASISizeBlock) anUploadSizeIncrementedBlock;
- (void)setDataReceivedBlock:(UA_ASIDataBlock)aReceivedBlock;
- (void)setAuthenticationNeededBlock:(UA_ASIBasicBlock)anAuthenticationBlock;
- (void)setProxyAuthenticationNeededBlock:(UA_ASIBasicBlock)aProxyAuthenticationBlock;
- (void)setRequestRedirectedBlock:(UA_ASIBasicBlock)aRedirectBlock;
#endif
...
+ (void)setDefaultCache:(id <UA_ASICacheDelegate>)cache;
...
+ (NSThread *)threadForRequest:(UA_ASIHTTPRequest *)request;
...
@property (assign) UA_ASIHTTPRequest *mainRequest;
...
@property (assign, readonly) UA_ASIAuthenticationState authenticationNeeded;
...
@property (assign) id <UA_ASICacheDelegate> downloadCache;
@property (assign) UA_ASICachePolicy cachePolicy;
@property (assign) UA_ASICacheStoragePolicy cacheStoragePolicy;
...
#endif
@property (retain) UA_ASIDataDecompressor *dataDecompressor;
