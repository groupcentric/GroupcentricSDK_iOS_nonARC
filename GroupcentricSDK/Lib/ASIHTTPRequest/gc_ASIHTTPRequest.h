...
@class gc_ASIDataDecompressor;

extern NSString *gc_ASIHTTPRequestVersion;
...
typedef enum _gc_ASIAuthenticationState {
	gc_ASINoAuthenticationNeededYet = 0,
	gc_ASIHTTPAuthenticationNeeded = 1,
	gc_ASIProxyAuthenticationNeeded = 2
} gc_ASIAuthenticationState;

typedef enum _gc_ASINetworkErrorType {
    gc_ASIConnectionFailureErrorType = 1,
    gc_ASIRequestTimedOutErrorType = 2,
    gc_ASIAuthenticationErrorType = 3,
    gc_ASIRequestCancelledErrorType = 4,
    gc_ASIUnableToCreateRequestErrorType = 5,
    gc_ASIInternalErrorWhileBuildingRequestType  = 6,
    gc_ASIInternalErrorWhileApplyingCredentialsType  = 7,
	gc_ASIFileManagementError = 8,
	gc_ASITooMuchRedirectionErrorType = 9,
	gc_ASIUnhandledExceptionError = 10,
	gc_ASICompressionError = 11
	
} gc_ASINetworkErrorType;


// The error domain that all errors generated by gc_ASIHTTPRequest use
extern NSString* const gc_NetworkRequestErrorDomain;
...
extern unsigned long const gc_ASIWWANBandwidthThrottleAmount;

#if NS_BLOCKS_AVAILABLE
typedef void (^gc_ASIBasicBlock)(void);
typedef void (^gc_ASIHeadersBlock)(NSDictionary *responseHeaders);
typedef void (^gc_ASISizeBlock)(long long size);
typedef void (^gc_ASIProgressBlock)(unsigned long long size, unsigned long long total);
typedef void (^gc_ASIDataBlock)(NSData *data);
#endif

@interface gc_ASIHTTPRequest : NSOperation <NSCopying> {
	...
	// The delegate - will be notified of various changes in state via the gc_ASIHTTPRequestDelegate protocol
	id <gc_ASIHTTPRequestDelegate> delegate;
	
	// Another delegate that is also notified of request status changes and progress updates
	// Generally, you won't use this directly, but gc_ASINetworkQueue sets itself as the queue so it can proxy updates to its own delegates
	// NOTE: WILL BE RETAINED BY THE REQUEST
	id <gc_ASIHTTPRequestDelegate, gc_ASIProgressDelegate> queue;
	...
	// Automatically set to true in gc_ASIFormDataRequests when using setFile:forKey:
	...
	// Set to true when gc_ASIHTTPRequest automatically created a temporary file containing the request body (when true, the file at postBodyFilePath will be deleted at the end of the request)
	...
	// If error code is = gc_ASIConnectionFailureErrorType (1, Connection failure occurred) - inspect [[error userInfo] objectForKey:NSUnderlyingErrorKey] for more information
	NSError *error;
	...
	id <gc_ASIProgressDelegate> uploadProgressDelegate;
	...
	id <gc_ASIProgressDelegate> downloadProgressDelegate;
	...
	// If you are using Basic authentication and want to force gc_ASIHTTPRequest to send an authorization header without waiting for a 401, you must set this to (NSString *)kCFHTTPAuthenticationSchemeBasic
	NSString *authenticationScheme;
	
	// Realm for authentication when credentials are required
	NSString *authenticationRealm;
	
	// When YES, gc_ASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a server that requires authentication
	...
	// When YES, gc_ASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a proxy server that requires authentication
	...
	// If you set this and implement the method in your delegate, you must handle the data yourself - gc_ASIHTTPRequest will not populate responseData or write the data to downloadDestinationPath
	...
	gc_ASIHTTPRequest *mainRequest;
	
	// When NO, this request will only update the progress indicator when it completes
	// When YES, this request will update the progress indicator according to how much data it has received so far
	// The default for requests is YES
	// Also see the comments in gc_ASINetworkQueue.h
	...
	// Tells gc_ASIHTTPRequest not to delete partial downloads, and allows it to use an existing file to resume a download. Defaults to NO.
	...
	// Details on the proxy to use - you could set these yourself, but it's probably best to let gc_ASIHTTPRequest detect the system proxy settings
	NSString *proxyHost;
	int proxyPort;
	
	// gc_ASIHTTPRequest will assume kCFProxyTypeHTTP if the proxy type could not be automatically determined
	...
	// See gc_ASIAuthenticationState values above. 0 == default == No authentication needed yet
	gc_ASIAuthenticationState authenticationNeeded;
	
	// When YES, gc_ASIHTTPRequests will present credentials from the session store for requests to the same server before being asked for them
	...
	// Temporarily set to YES when a closed connection forces a retry (internally, this stops gc_ASIHTTPRequest cleaning up a temporary post body)
	...
	// Will be gc_ASIHTTPRequestRunLoopMode for synchronous requests, NSDefaultRunLoopMode for all other requests
	NSString *runLoopMode;
	
	// This timer checks up on the request every 0.25 seconds, and updates progress
	NSTimer *statusTimer;
	
	// The download cache that will be used for this request (use [gc_ASIHTTPRequest setDefaultCache:cache] to configure a default cache
	id <gc_ASICacheDelegate> downloadCache;
	
	// The cache policy that will be used for this request - See gc_ASICacheDelegate.h for possible values
	gc_ASICachePolicy cachePolicy;
	
	// The cache storage policy that will be used for this request - See gc_ASICacheDelegate.h for possible values
	gc_ASICacheStoragePolicy cacheStoragePolicy;
	...
	gc_ASIDataDecompressor *dataDecompressor;
	...
	//
	// Setting this to NO may be especially useful for users using gc_ASIHTTPRequest in conjunction with a streaming parser, as it will allow partial gzipped responses to be inflated and passed on to the parser while the request is still running
	...
	gc_ASIHTTPRequest *PACFileRequest;
	...
	gc_ASIBasicBlock startedBlock;

	//block to execute when headers are received
	gc_ASIHeadersBlock headersReceivedBlock;

	//block to execute when request completes successfully
	gc_ASIBasicBlock completionBlock;

	//block to execute when request fails
	gc_ASIBasicBlock failureBlock;

	//block for when bytes are received
	gc_ASIProgressBlock bytesReceivedBlock;

	//block for when bytes are sent
	gc_ASIProgressBlock bytesSentBlock;

	//block for when download size is incremented
	gc_ASISizeBlock downloadSizeIncrementedBlock;

	//block for when upload size is incremented
	gc_ASISizeBlock uploadSizeIncrementedBlock;

	//block for handling raw bytes received
	gc_ASIDataBlock dataReceivedBlock;

	//block for handling authentication
	gc_ASIBasicBlock authenticationNeededBlock;

	//block for handling proxy authentication
	gc_ASIBasicBlock proxyAuthenticationNeededBlock;
	
    //block for handling redirections, if you want to
    gc_ASIBasicBlock requestRedirectedBlock;
	#endif
}
...
// Used internally to record bandwidth use, and by gc_ASIInputStreams when uploading. It's probably best if you don't mess with this.
+ (void)incrementBandwidthUsedInLastSecond:(unsigned long)bytes;

// On iPhone, gc_ASIHTTPRequest can automatically turn throttling on and off as the connection type changes between WWAN and WiFi
...
@property (assign) gc_ASIHTTPRequest *mainRequest;
...
@property (assign, readonly) gc_ASIAuthenticationState authenticationNeeded;
...
@property (assign) id <gc_ASICacheDelegate> downloadCache;
@property (assign) gc_ASICachePolicy cachePolicy;
@property (assign) gc_ASICacheStoragePolicy cacheStoragePolicy;
...
#endif
@property (retain) gc_ASIDataDecompressor *dataDecompressor;
