...
static gc_ASIAuthenticationDialog *sharedDialog = nil;
BOOL gc_isDismissing = NO;
...
@implementation gc_ASIAutorotatingViewController
...
@interface gc_ASIAuthenticationDialog ()
- (void)showTitle;
- (void)show;
...
@implementation gc_ASIAuthenticationDialog

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [gc_ASIAuthenticationDialog class]) {
		requestsNeedingAuthentication = [[NSMutableArray array] retain];
	}
}

+ (void)presentAuthenticationDialogForRequest:(gc_ASIHTTPRequest *)theRequest
{
	...
	if (!sharedDialog) {
		...
		if ([theRequest authenticationNeeded] == gc_ASIProxyAuthenticationNeeded) {
			[sharedDialog setType:gc_ASIProxyAuthenticationType];
		} else {
			[sharedDialog setType:gc_ASIStandardAuthenticationType];
		}
		[sharedDialog show];
	} else {
...
- (UIViewController *)presentingController
{
	if (!presentingController) {
		presentingController = [[gc_ASIAutorotatingViewController alloc] initWithNibName:nil bundle:nil];
...
- (void)showTitle
{
	...
	if (UIInterfaceOrientationIsPortrait([[UIDevice currentDevice] orientation])) {
		// Setup the title
		if ([self type] == gc_ASIProxyAuthenticationType) {
...
- (void)cancelAuthenticationFromDialog:(id)sender
{
	for (gc_ASIHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
...
- (NSArray *)requestsRequiringTheseCredentials
{
	...
	for (gc_ASIHTTPRequest *otherRequest in requestsNeedingAuthentication) {
...
- (void)presentNextDialog
{
	if ([requestsNeedingAuthentication count]) {
		gc_ASIHTTPRequest *nextRequest = [requestsNeedingAuthentication objectAtIndex:0];
...
- (void)loginWithCredentialsFromDialog:(id)sender
{
	for (gc_ASIHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
		...
		// Handle NTLM domains
		NSString *scheme = ([self type] == gc_ASIStandardAuthenticationType) ? [[self request] authenticationScheme] : [[self request] proxyAuthenticationScheme];
