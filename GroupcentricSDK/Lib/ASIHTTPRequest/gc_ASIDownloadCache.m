...
#import "gc_ASIDownloadCache.h"
#import "gc_ASIHTTPRequest.h"
#import <CommonCrypto/CommonHMAC.h>

static gc_ASIDownloadCache *sharedCache = nil;

static NSString *sessionCacheFolder = @"SessionStore";
static NSString *permanentCacheFolder = @"PermanentStore";

@interface gc_ASIDownloadCache ()
+ (NSString *)keyForURL:(NSURL *)url;
- (NSString *)pathToFile:(NSString *)file;
@end

@implementation gc_ASIDownloadCache

- (id)init
{
	self = [super init];
	[self setShouldRespectCacheControlHeaders:YES];
	[self setDefaultCachePolicy:gc_ASIUseDefaultCachePolicy];
	...
	if (!sharedCache) {
		@synchronized(self) {
			if (!sharedCache) {
				...
				[sharedCache setStoragePath:[[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingPathComponent:@"gc_ASIHTTPRequestCache"]];
			}
		}
	}
	return sharedCache;
...
{
	[[self accessLock] lock];
	[self clearCachedResponsesForStoragePolicy:gc_ASICacheForSessionDurationCacheStoragePolicy];
	[storagePath release];
...
- (void)updateExpiryForRequest:(gc_ASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	[cachedHeaders setObject:[NSNumber numberWithDouble:[expires timeIntervalSince1970]] forKey:@"X-gc_ASIHTTPRequest-Expires"];
...
- (NSDate *)expiryDateForRequest:(gc_ASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if (maxAge) {
		...
		if (expires) {
			return [gc_ASIHTTPRequest dateFromRFC1123String:expires];
		}
	}
	return nil;
}
...
- (void)storeResponseForRequest:(gc_ASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if ([request error] || ![request responseHeaders] || ([request cachePolicy] & gc_ASIDoNotWriteToCacheCachePolicy)) {
		[[self accessLock] unlock];
		return;
	}
	...
	// Create a special 'X-gc_ASIHTTPRequest-Expires' header
	...
	NSDate *expires = [self expiryDateForRequest:request maxAge:maxAge];
	if (expires) {
		[responseHeaders setObject:[NSNumber numberWithDouble:[expires timeIntervalSince1970]] forKey:@"X-gc_ASIHTTPRequest-Expires"];
	}
	...
	[responseHeaders setObject:[NSNumber numberWithInt:statusCode] forKey:@"X-gc_ASIHTTPRequest-Response-Status-Code"];
...
- (NSString *)pathToStoreCachedResponseDataForRequest:(gc_ASIHTTPRequest *)request
{
	...
	NSString *path = [[self storagePath] stringByAppendingPathComponent:([request cacheStoragePolicy] == gc_ASICacheForSessionDurationCacheStoragePolicy ? sessionCacheFolder : permanentCacheFolder)];
...
- (BOOL)isCachedDataCurrentForRequest:(gc_ASIHTTPRequest *)request
{
	...
	if ([self shouldRespectCacheControlHeaders]) {
		...
		// Look for X-gc_ASIHTTPRequest-Expires header to see if the content is out of date
		NSNumber *expires = [cachedHeaders objectForKey:@"X-gc_ASIHTTPRequest-Expires"];
		if (expires) {
...
{
	[[self accessLock] lock];
	gc_ASICachePolicy cp = defaultCachePolicy;
	[[self accessLock] unlock];
	return cp;
}


- (void)setDefaultCachePolicy:(gc_ASICachePolicy)cachePolicy
{
	[[self accessLock] lock];
	if (!cachePolicy) {
		defaultCachePolicy = gc_ASIAskServerIfModifiedWhenStaleCachePolicy;
	}  else {
...
- (void)clearCachedResponsesForStoragePolicy:(gc_ASICacheStoragePolicy)storagePolicy
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
	...
	NSString *path = [[self storagePath] stringByAppendingPathComponent:(storagePolicy == gc_ASICacheForSessionDurationCacheStoragePolicy ? sessionCacheFolder : permanentCacheFolder)];
...
+ (NSString *)keyForURL:(NSURL *)url
{
	...
	// Strip trailing slashes so http://allseeing-i.com/gc_ASIHTTPRequest/ is cached the same as http://allseeing-i.com/gc_ASIHTTPRequest
...
- (BOOL)canUseCachedDataForRequest:(gc_ASIHTTPRequest *)request
{
	...
	if ([request cachePolicy] & gc_ASIDoNotReadFromCacheCachePolicy) {
		return NO;

	// If we don't want to load the request whatever happens, always pretend we have cached data even if we don't
	} else if ([request cachePolicy] & gc_ASIDontLoadCachePolicy) {
		return YES;
	}
	...
	if ([request cachePolicy] & gc_ASIOnlyLoadIfNotCachedCachePolicy) {
		return YES;

	// If we want to fallback to the cache after an error
	} else if ([request complete] && [request cachePolicy] & gc_ASIFallbackToCacheIfLoadFailsCachePolicy) {
		return YES;

	// If we have cached data that is current, we can use it
	} else if ([request cachePolicy] & gc_ASIAskServerIfModifiedWhenStaleCachePolicy) {
	...
	} else if ([request cachePolicy] & gc_ASIAskServerIfModifiedCachePolicy) {
