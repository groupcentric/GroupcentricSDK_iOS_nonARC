...
@implementation gc_ASIInputStream

+ (void)initialize
{
	if (self == [gc_ASIInputStream class]) {
		readLock = [[NSLock alloc] init];
	}
}

+ (id)inputStreamWithFileAtPath:(NSString *)path request:(gc_ASIHTTPRequest *)theRequest
{
	gc_ASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
	[theStream setStream:[NSInputStream inputStreamWithFileAtPath:path]];
	return theStream;
}

+ (id)inputStreamWithData:(NSData *)data request:(gc_ASIHTTPRequest *)theRequest
{
	gc_ASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
...
// When throttling is on, we ask gc_ASIHTTPRequest for the maximum amount of data we can read
- (NSInteger)read:(uint8_t *)buffer maxLength:(NSUInteger)len
{
	[readLock lock];
	unsigned long toRead = len;
	if ([gc_ASIHTTPRequest isBandwidthThrottled]) {
		toRead = [gc_ASIHTTPRequest maxUploadReadLength];
		if (toRead > len) {
			toRead = len;
		} else if (toRead == 0) {
			toRead = 1;
	...
	[gc_ASIHTTPRequest incrementBandwidthUsedInLastSecond:toRead];
	[readLock unlock];
